...
if __name__ == "__main__":
    ...
    # print(var1)
    plt.errorbar(x, mean1, 2 * var1, fmt='r--', capsize=2.5, errorevery=10, markevery=10, label=r"S-TSQL-$\theta_n=\frac{10}{n+100}$")
    plt.errorbar(x, mean2, 2 * var2, fmt='b--', capsize=2.5, markevery=10, errorevery=10, label=r"S-TSQL-$\theta_n=\frac{(-1)^n}{n^2+10}$")
    ...
    plt.errorbar(x, mean5, 2 * var5, fmt='g--', capsize=2.5, markevery=10, errorevery=10,
                  label=r"S-TSQL-$\theta_n=\frac{-10}{n+100}$")
    plt.errorbar(x, mean9, 2 * var9, fmt='y--', capsize=2.5, markevery=10, errorevery=10,
                  label=r"S-TSQL-$\theta_n=\frac{1}{\sqrt{n+1}}$")
    ...
    plt.tight_layout()
    plt.savefig('S-TSQL-theta.pdf', dpi=600, bbox_inches='tight')
    plt.close()
